<?php  
  ...
  class Validate{
    ...
    public function __construct($fname,$lname,$file_name,$phn){
      $this->fname=$fname;
      $this->lname=$lname;
    ...
    public function vaildPhn(){
      $flag=true;
      if(!preg_match('/^[0-9]{10}$/', $this->phn)) {
          $this->phnErr="not a valid number";
          $flag=false;
        }
      return $flag;
    }
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])){
    $obj=new Validate($_POST["fname"],$_POST["lname"],$_FILES["image-upload"]["name"],$_POST["phn"]);
    $obj->isEmpty();
    $obj->isAlpha();
    $obj->imgType();
    $obj->validPhn();
    if($obj->isEmpty() && $obj->isAlpha() && $obj->imgType() && $obj->vaildPhn()){
      ...
      <div class="container">
        ...
        <form method="POST"  action="<?php echo $_SERVER["PHP-SELF"]; ?>" enctype="multipart/form-data">
          ...
          <div class="input-field fullname">
            <span>FULL NAME :</span> <input type="text" name="fullname" placeholder="your full name" value="<?php echo isset($_POST['fullname']) ? ($fname." ".$lname) : '' ?>" disabled>
          </div>
          ...
          <div class="input-field phn">
            <span>PHONE NUMBER :</span> <input type="text" name="phn" value="<?php echo isset($_POST['phn']) ? $obj->phn : ''; ?>" placeholder="enter your mobile number">
